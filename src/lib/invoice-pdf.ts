...
export async function buildInvoicePdf(invoice: InvoiceWith, company: string): Promise<Uint8Array> {
  ...
  page.drawText(safe("PERIOD"), { x: colR, y, size: 8, font: bold, color: muted });
  y -= 13;
  text(invoice.customer?.name ?? "—", M, 10.5, bold);
  page.drawText(safe(`${invoice.periodStart} to ${invoice.periodEnd}`), {
  ...
  page.drawText(
    safe(
      `Terms: ${invoice.customer?.paymentTerms || "-"}` +
  ...
  if (invoice.retainageHeld > 0) {
    totalRow(
      `Retainage (${+(invoice.retainagePct * 100).toFixed(2)}%)`,
      `-${money(invoice.retainageHeld)}`,
    );
  }
